...
namespace _1912901057_AtakanCetinkaya
{
    public partial class UyeDetay : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if(!IsPostBack)
            {
                ...
                else
                {
                    ...
                    DataTable dt = VeriTabani.veriGetir("select * from kullanici where uyeNo = " + Session["DetayUyeNo"].ToString());
                    if (dt.Rows.Count > 0)
                    {
        ...
        protected void btnUyeEkle_Click(object sender, EventArgs e)
        {
            ...
            {
                OleDbCommand cmd = VeriTabani.KomutOlustur("insert into kullanici (kullaniciAdi, parola, adi, soyadi, eposta, yetki)" + "values (@kullaniciAdi, @parola, @adi, @soyadi, @eposta, @yetki)");
                ...
                cmd.Parameters.AddWithValue("@yetki", drpYetki.SelectedValue);
        ...
        protected void btnGuncelle_Click(object sender, EventArgs e)
        {
            if (lblGuvenlik.Text == txtGuvenlik.Text)
            {
                OleDbCommand cmd = VeriTabani.KomutOlustur("update kullanici set kullaniciadi = @kullaniciAdi, parola = @parola, adi = @adi, soyadi = @soyadi, " + "eposta = @eposta, yetki = @yetki where uyeNo = @uyeNo");
